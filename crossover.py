...
def cross_populate(
    parent_list, num_of_seeds, inheritence_rate=100, mutation_rate=1000
):  # takes an input of form [('word', 10.3) ('word2', 20) ... ('word_n', score_n)]
    ...
    for child in range(num_of_seeds):
        ...
        for letter in range(len(parent_list[0][0])):
            for fitness in range(len(parent_list)):
                ...
                suncream = randint(1, mutation_rate) < mutation_rate
                if mouse <= parent_list[fitness][1] and suncream:
