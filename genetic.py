...
from EnuBubbleSort import bubble_sort #?? Is the syntax for importing a function from a .py file the same as the syntax for importing a class from a .py file?
from crossover import cross_populate

target = "the lazy dog jumped over the quick brown fox"
seed_count = 1000        #number of starting phrases
parent_count = 2        # number of parents per generation
inheritence_rate = 60  #lower => more of the
mutation_rate = 100
initialPopulation= Population(len(target), seed_count).initialise() #make a population object
...
fitnesses = Evaluator(initialPopulation, target).fitness_percent()
...
count = 0
while newParents[-1][0] != target:
    child_fitnesses = Evaluator(children, target).fitness_percent()
